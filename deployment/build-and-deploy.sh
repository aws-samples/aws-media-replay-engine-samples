...
if [ "$app" = "plugin-samples" ]; then
  ...
  # Remove leading and trailing double-quotes in the helper arn
  ...
  echo "Finished deploying the Plugins stack"

  echo "Registering the Plugins in MRE"
  (python3 register_plugin.py $region $(if [ ! -z $profile ]; then echo "$profile"; fi)) 2>&1
  if [ $? -ne 0 ]; then
      echo "ERROR: Failed to register the Plugins in MRE."
      exit 1
  fi
  echo "Finished registering the Plugins in MRE"
